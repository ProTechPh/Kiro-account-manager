...
### 🛡️ 高级代理引擎
- **智能熔断器**: 指数退避冷却 + 概率性重试，多账号智能故障转移
- **可配置超时**: 首 Token 超时、流式读取超时、最大重试次数
- **Payload 大小保护**: 可配置 payload 限制，支持自动裁剪或返回错误
- **思考预算上限**: 防止模型将所有输出 token 花在推理上
...
**新功能:**
- 🛡️ **熔断器指数退避**: 智能冷却 `base × 2^(failures-1)` + 概率性重试（10%），防止账号永久卡死
- ⏱️ **可配置流式超时**: 首 Token 超时（15s）、最大重试（3次）、流式读取超时（5分钟）— 全部可在 UI 调整
- 🔍 **Web Search 工具自动注入**: MCP 模拟 — 自动注入 `web_search` 工具让模型可以搜索网络
- 📦 **Payload 大小保护**: 可配置限制（默认 600KB）+ 自动裁剪开关 — 不再静默丢失数据
- 🧠 **思考预算上限**: 默认预算（4000 tokens）+ 最大上限（10000），防止模型将所有输出花在推理上
- 🖥️ **10 个新 UI 设置项**: 所有功能均在高级设置面板中暴露，完整中英文翻译

完整说明见: `docs/CHANGELOG-v1.7.1.md`
...
**Bug 修复:**
- 🐛 **AI 响应中回显占位符文本**: 修复了 AI 模型将助手历史消息中的任意占位符文本（如 `"Using tools."`、`"(tool_call)"`）原样回显到实际响应中的问题。现已彻底移除包含工具调用的助手消息中的所有占位符内容。

完整说明见: `docs/CHANGELOG-v1.6.9.md`
