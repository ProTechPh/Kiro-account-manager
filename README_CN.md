...
### ⚡ 启动与自动修复
- **开机自启动**: 系统启动时自动运行应用
- **自动启动服务器**: 应用打开时自动启动代理服务器
- **自动修复**: 启动/重启时自动连通性自检 — 检测到 401 时自动轮换 Proxy API Key，服务无响应时自动重启
...
**新功能：**
- ⚡ **开机自启动**: 系统启动时自动运行应用（原生系统登录项）
- 🚀 **自动启动服务器**: 应用打开时自动启动代理服务器（设置页面开关）
- 🛠️ **自动修复**: 启动/重启时连通性自检 — 探测 `/v1/models`，401 时自动轮换 API Key，服务无响应时自动重启

详见: `docs/CHANGELOG-v1.8.1.md`
