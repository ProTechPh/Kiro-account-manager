...
export function GroupManageDialog({ isOpen, onClose }: GroupManageDialogProps): React.ReactNode {
  ...
  const handleDelete = (id: string, name: string) => {
    ...
    const msg = count > 0
      ? t('groupManage.deleteConfirmWithAccounts', { name, count })
      : t('groupManage.deleteConfirm', { name })
    if (confirm(msg)) {
      removeGroup(id)
    }
  }
      ...
      <Card className="relative w-full max-w-2xl max-h-[85vh] overflow-hidden z-10 flex flex-col">
        ...
        <CardContent className="flex-1 overflow-auto space-y-4">
          ...
          <div className="flex gap-4 text-sm text-muted-foreground">
            <span>{t('groupManage.groupCount', { count: groupList.length })}</span>
            <span>•</span>
            <span>{t('groupManage.ungroupedCount', { count: getUngroupedCount() })}</span>
          </div>
          ...
          {isCreating ? (
            <div className="p-4 border rounded-lg space-y-3 bg-muted/30">
              <div className="flex items-center gap-2">
                ...
                <input
                  type="text"
                  placeholder={t('groupManage.namePlaceholder')}
                  value={newName}
              ...
              <input
                type="text"
                placeholder={t('groupManage.descPlaceholder')}
          ...
          <div className="space-y-2">
              ...
              >
                {editingId === group.id ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      type="text"
                      placeholder={t('groupManage.descEditPlaceholder')}
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-muted-foreground">{t('groupManage.selectAccounts')}</span>
                    </div>
                    
                    {/* 当前分组内的账号 */}
                    {getGroupAccounts(group.id).length > 0 && (
                      <div className="space-y-1">
                        <p className="text-xs text-muted-foreground">{t('groupManage.currentAccounts')}</p>
                          ...
                          {getGroupAccounts(group.id).map(acc => (
                            ...
                            >
                              ...
                              <button
                                ...
                                title={t('groupManage.removeFromGroup')}
                              >
                                <X className="h-3 w-3" />
                              </button>
                            </span>
                    ...
                    {getAssignableAccounts(group.id).length > 0 && (
                      <div className="space-y-1">
                        <p className="text-xs text-muted-foreground">{t('groupManage.clickToAdd')}</p>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <Button
                        ...
                        title={t('groupManage.manageAccounts')}
                      >
            ...
            {groupList.length === 0 && !isCreating && (
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p>{t('groupManage.noGroups')}</p>
                <p className="text-sm">{t('groupManage.noGroupsHint')}</p>
              </div>
            )}
          </div>
        </CardContent>
