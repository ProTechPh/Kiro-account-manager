...
export function AccountToolbar({
  ...
  const handleBatchDelete = (): void => {
    if (selectedCount === 0) return
    if (confirm(t('toolbar.deleteConfirm', { count: selectedCount }))) {
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4 text-sm">
          <span className="text-muted-foreground">
            {t('toolbar.statsTotal', { count: stats.total })}
            {filteredCount !== stats.total && (
              <span>{t('toolbar.statsFiltered', { count: filteredCount })}</span>
            )}
          </span>
          {stats.expiringSoonCount > 0 && (
            <Badge variant="destructive" className="gap-1">
              {stats.expiringSoonCount} {t('toolbar.expiring')}
            </Badge>
          )}
        </div>
        ...
        <div className="flex items-center gap-1">
          ...
          <div className="relative" ref={groupMenuRef}>
            <Button 
              ...
              title={selectedCount > 0 ? t('toolbar.setGroup') : t('toolbar.manageGroups')}
            >
            ...
            {showGroupMenu && selectedCount > 0 && (
              <div className="absolute left-0 top-full mt-2 z-50 min-w-[200px] bg-popover border rounded-lg shadow-lg p-2">
                ...
                >
                  ...
                  <span>{t('toolbar.removeGroup')}</span>
                  {(() => {
                ...
                >
                  ...
                  <span>{t('toolbar.manageGroups')}</span>
                </button>
              </div>
            )}
          </div>
          ...
          <div className="relative" ref={tagMenuRef}>
            <Button 
              ...
              title={selectedCount > 0 ? t('toolbar.setTags') : t('toolbar.manageTags')}
            >
            ...
            {showTagMenu && selectedCount > 0 && (
              <div className="absolute left-0 top-full mt-2 z-50 min-w-[220px] bg-popover border rounded-lg shadow-lg p-2">
                <div className="absolute -top-2 left-4 w-4 h-4 bg-popover border-l border-t rotate-45" />
                <div className="text-xs text-muted-foreground px-2 py-1 mb-1">
                  {t('toolbar.selectedMulti', { count: selectedCount })}
                </div>
                <div className="border-t my-1" />
                ...
                >
                  <Plus className="h-4 w-4" />
                  <span>{t('toolbar.manageTags')}</span>
                </button>
              </div>
            )}
          </div>
          <Button
            ...
            title={privacyMode ? t('toolbar.disablePrivacy') : t('toolbar.enablePrivacy')}
          >
            {privacyMode ? (
          ...
          <Button
            ...
            title={t('toolbar.refreshTokenTitle')}
          >
            {isRefreshing ? (
              <Loader2 className="h-4 w-4 mr-1 animate-spin" />
            ) : (
          ...
          >
            ...
            {selectedCount > 0 ? t('toolbar.selectedShort', { count: selectedCount }) : t('toolbar.selectAll')}
          </Button>
        </div>
      </div>
    </div>
