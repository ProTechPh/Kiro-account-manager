...
export function ModelMappingDialog({
  ...
  const getTypeLabel = (type: string) => {
    switch (type) {
      case 'replace': return t('modelMapping.typeReplace')
      case 'alias': return t('modelMapping.typeAlias')
      case 'loadbalance': return t('modelMapping.typeLoadBalance')
      default: return type
    }
  }
          ...
          <div className="max-h-[calc(85vh-180px)] overflow-y-auto pr-2 space-y-3">
            {localMappings.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
                ...
                <p className="text-sm mt-1">{t('modelMapping.noRulesHint')}</p>
              </div>
            ) : (
                ...
                >
                  ...
                  {expandedId === rule.id && (
                    <div className="border-t p-4 space-y-4 bg-muted/10">
                      <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            placeholder={t('modelMapping.ruleNamePlaceholder')}
                          />
                        </div>
                        <div className="space-y-2">
                          ...
                          >
                            <option value="replace">{t('modelMapping.typeReplaceDesc')}</option>
                            <option value="alias">{t('modelMapping.typeAliasDesc')}</option>
                            <option value="loadbalance">{t('modelMapping.typeLoadBalanceDesc')}</option>
                          </select>
                        </div>
                      </div>
                      ...
                      <div className="space-y-2">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground mt-0.5">
                            {t('modelMapping.sourceModelDesc')}
                          </p>
                        </div>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground mt-0.5">
                              {t('modelMapping.targetModelsDesc')}
                            </p>
                          </div>
                          <Button 
                            variant="outline" 
                          ...
                          {rule.targetModels.map((target, targetIndex) => (
                            <div key={targetIndex} className="flex items-center gap-2">
                              <Input 
                                ...
                                placeholder={t('modelMapping.targetModelPlaceholder')}
                                className="flex-1"
                              />
                              {availableModels.length > 0 && (
                                <select
                      ...
                      {apiKeys.length > 0 && (
                        <div className="space-y-2">
                          <Label>{t('modelMapping.apiKeysLabel')}</Label>
