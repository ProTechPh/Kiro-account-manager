      ...
      <Card className="relative w-[900px] max-h-[80vh] shadow-2xl border-0 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <CardHeader className="pb-3 border-b sticky top-0 bg-background z-10">
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-sm">
            <span>{t('proxyLogs.total')}: <Badge variant="secondary">{logs.length}</Badge></span>
            <span>{t('proxyLogs.success')}: <Badge className="bg-green-500/20 text-green-600">{successCount}</Badge></span>
            <span>{t('proxyLogs.error')}: <Badge className="bg-red-500/20 text-red-600">{errorCount}</Badge></span>
            <span className="text-muted-foreground">|</span>
            <span>Tokens {t('proxyLogs.recent')}: <Badge variant="outline">{recentTokens.toLocaleString()}</Badge></span>
            <span className="flex items-center gap-1">
              Tokens {t('proxyLogs.total')}: <Badge variant="secondary">{totalTokens.toLocaleString()}</Badge>
              {onResetTokens && (
                <Button variant="ghost" size="icon" className="h-5 w-5" onClick={onResetTokens} title={t('proxyLogs.resetTotal')}>
                  <RotateCcw className="h-3 w-3" />
                </Button>
              )}
            </span>
            <span className="text-muted-foreground">|</span>
            <span>Credits {t('proxyLogs.recent')}: <Badge variant="outline">{recentCredits.toFixed(4)}</Badge></span>
            <span className="flex items-center gap-1">
              Credits {t('proxyLogs.total')}: <Badge variant="secondary">{totalCredits.toFixed(4)}</Badge>
              {onResetCredits && (
                <Button variant="ghost" size="icon" className="h-5 w-5" onClick={onResetCredits} title={t('proxyLogs.resetTotal')}>
                  <RotateCcw className="h-3 w-3" />
                </Button>
              )}
            </span>
            ...
            {logs.length === 0 ? (
              ...
              <table className="w-full text-sm">
                <thead className="bg-muted/50 sticky top-0">
                  <tr>
                    <th className="text-left p-2 font-medium">{t('apiKeyUsage.time')}</th>
                    <th className="text-left p-2 font-medium">{t('apiKeyUsage.path')}</th>
                    <th className="text-left p-2 font-medium">{t('apiKeyUsage.model')}</th>
                    <th className="text-center p-2 font-medium">{t('proxyLogs.status')}</th>
                    <th className="text-center p-2 font-medium">{t('apiKeyUsage.in')}</th>
                    <th className="text-center p-2 font-medium">{t('apiKeyUsage.out')}</th>
